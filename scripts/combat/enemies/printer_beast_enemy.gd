# printer_beast_enemy.gd
# The "Printer Beast" enemy - a hostile print daemon
extends Node
class_name PrinterBeastEnemy
...
@export_group("Printer Beast Stats")
@export var enemy_name: String = "Printer Boss"
@export var max_hp: int = 120
@export var attack_power: int = 17
@export var defense: int = 7
@export var speed: int = 6

@export_group("Encounter Settings")
@export var allow_puzzle_solution: bool = true
@export var puzzle_unlocks_at_hp_percent: float = 60.0
@export var hostile_start: bool = true

@export_group("Dialogue")
@export var dialogue_json_path: String = "res://dialogues/enemies/printer_beast.json"
...
func _load_dialogue_files() -> void:
	...
	intro_lines = [
		"PCL XL Error!",
		"Paper jam. Spool overflow. Memory fault.",
		"[paper shuriken whirl]",
	]

	puzzle_offer_lines = [
		"Clear the jam. Reset the spooler.",
		"Rebuild the queue and the daemon will quiet.",
		"[TYPE 'puzzle' to clear the printer, or continue fighting]",
	]

	defeat_lines = [
		"[ERROR: SPOOL HALT]",
		"The paper storm collapses.",
	]

	puzzle_success_lines = [
		"[quiet whirr] The queue stabilizes.",
		"The printer daemon is silent.",
	]
#endregion
...
func _initialize_enemy_data() -> void:
	...
	enemy_data.weakness = "patch"
	enemy_data.resistance = "copy"
	enemy_data.abilities = ["paper_shuriken", "jam", "spool_burst"]
	enemy_data.description = "A massive printer daemon roaring error codes and flinging paper shuriken."
...
func _handle_dialogue_input(input: String) -> Dictionary:
	...
	if input == "continue" or input == "next" or input == "":
		...
		if dialogue_index >= intro_lines.size():
			result.message = "[The Printer Beast roars and waits...]\n"
			result.message += "[Type 'attack' to fight, or 'puzzle' to clear the jam]"
		else:
			result.message = intro_lines[dialogue_index]
			if dialogue_index < intro_lines.size() - 1:
				result.message += "\n[Type 'continue' to proceed, 'attack' to fight, or 'puzzle' to clear the jam]"
			else:
				result.message += "\n[Type 'attack' to fight, or 'puzzle' to clear the jam]"
	elif input == "attack" or input == "fight":
		has_attacked = true
		_transition_to_combat()
		result.mode_changed = true
		result.message = "[Combat initiated - paper shuriken incoming]"
	elif input == "puzzle" or input == "help" or input == "clear":
		if allow_puzzle_solution:
			_transition_to_puzzle()
			result.mode_changed = true
			result.message = "[Puzzle mode initiated - you attempt to clear the printer]"
		else:
			result.message = "The printer daemon is too violent to clear right now..."
	else:
...
func _handle_combat_input(input: String) -> Dictionary:
	...
	if input == "puzzle" or input == "help" or input == "clear" or input == "restore":
		if has_attacked:
			result.message = "[You've already chosen to fight. The daemon resists repairs.]"
			return result
	...
	if not puzzle_offered and not has_attacked and _should_offer_puzzle():
		puzzle_offered = true
		result.message = "[The printer's roar stutters...]\n"
		for line in puzzle_offer_lines:
			result.message += line + "\n"
		result.message += "\n[Type 'puzzle' to clear the jam, or continue fighting]"
		return result
...
func _handle_patch_in_combat() -> String:
	...
	var msg := "[PATCH command disrupts the spooler!]\n"
	msg += "The queue jitters, taking %d integrity damage!\n" % damage
	...
	if enemy_data.current_hp <= 0:
		...
		msg += "Printer Beast HP: %d/%d" % [enemy_data.current_hp, enemy_data.max_hp]
...
func _resolve_encounter(method: String) -> void:
	...
	match method:
		"puzzle_solved":
			...
			if granted_printer_key:
				_queue_key_reward_popup("Printer Proficiency Key")
			_maybe_complete_drivers_den_quest()
		"combat_victory":
			var granted_printer_key_combat := not SceneManager.proficiency_key_printer
